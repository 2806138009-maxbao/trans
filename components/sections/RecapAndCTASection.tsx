  ...
  return (
    <section id={id} className="w-full relative px-6 py-16">
      ...
      <div className="relative max-w-6xl mx-auto space-y-12">
        ...
        {/* Student & Teacher Cards - Void Style */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Students */}
          <Wrapper {...getWrapperProps('slide-left', 200)}>
            <div className="relative pl-6 py-6 group">
              ...
              <div 
                ...
                style={{ backgroundColor: '#479CFF' }}
              />
              
              <div className="space-y-6">
                <div className="flex items-center gap-3">
                  <span className="w-10 h-10 rounded-xl bg-[#479CFF]/10 flex items-center justify-center">
                    <svg className="w-5 h-5 text-[#479CFF]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                  </svg>
                </span>
                  <h3 className="text-xl font-semibold text-white">{t.recapStudentsTitle}</h3>
              </div>
                <p className="text-base text-[#C7CBD4] leading-relaxed">{t.recapStudentsLead}</p>
                <ul className="space-y-3">
                  {t.recapStudentsBullets.map((line, idx) => (
                    <li key={idx} className="flex gap-3 text-sm text-[#D0D6E0] leading-relaxed">
                      <span className="text-[#479CFF] flex-shrink-0 font-medium">{idx + 1}.</span>
          ...
          {/* Teachers */}
          <Wrapper {...getWrapperProps('slide-right', 300)}>
            <div className="relative pl-6 py-6 group">
              {/* Left accent line */}
              <div 
                className="absolute top-0 bottom-0 left-0 w-0.5 transition-all duration-300 group-hover:w-1"
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-3">
                  <span className="w-10 h-10 rounded-xl bg-[#FFC700]/10 flex items-center justify-center">
                    <svg className="w-5 h-5 text-[#FFC700]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                  </svg>
                </span>
                  <h3 className="text-xl font-semibold text-white">{t.recapTeachersTitle}</h3>
              </div>
                <p className="text-base text-[#C7CBD4] leading-relaxed">{t.recapTeachersLead}</p>
                <ul className="space-y-3">
                  {t.recapTeachersBullets.map((line, idx) => (
                    <li key={idx} className="flex gap-3 text-sm text-[#D0D6E0] leading-relaxed">
                      <span className="text-[#FFC700] flex-shrink-0 font-medium">{idx + 1}.</span>
                      <span className="leading-relaxed">{line}</span>
                  </li>
                ))}
              </ul>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Wrapper {...getWrapperProps('slide-left', 200)}>
            <TiltCard glowColor="rgba(71,156,255,0.4)">
              <div className="p-6 space-y-4">
                ...
                <p className="text-sm text-[#C7CBD4] leading-relaxed">{t.recapStudentsLead}</p>
                <ul className="space-y-2">
                  {t.recapStudentsBullets.map((line, idx) => (
                    <li key={idx} className="flex gap-2 text-sm text-[#D0D6E0]">
          ...
          {/* Teachers */}
          <Wrapper {...getWrapperProps('slide-right', 300)}>
            <TiltCard glowColor="rgba(255,199,0,0.4)">
              <div className="p-6 space-y-4">
                <div className="flex items-center gap-2">
                  <span className="w-8 h-8 rounded-lg bg-[#FFC700]/10 flex items-center justify-center">
                ...
                <p className="text-sm text-[#C7CBD4] leading-relaxed">{t.recapTeachersLead}</p>
                <ul className="space-y-2">
                  {t.recapTeachersBullets.map((line, idx) => (
                    <li key={idx} className="flex gap-2 text-sm text-[#D0D6E0]">
                      <span className="text-[#FFC700] flex-shrink-0">{idx + 1}.</span>
                      <span className="leading-relaxed">{line}</span>
                    </li>
                  ))}
                </ul>
          ...
          <div className="text-center space-y-6">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {t.nextItems.map((item, idx) => (
                <div 
                  key={idx}
                  className="px-4 py-2 rounded-full bg-white/5 border border-white/10 text-sm text-[#D0D6E0] hover:border-[#FFC700]/30 hover:text-white transition-all duration-300"
                >
                  {item}
                </div>
              ))}
