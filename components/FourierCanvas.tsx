...
export const FourierCanvas: React.FC<FourierCanvasProps> = ({ nVal, lang }) => {
  ...
  useEffect(() => {
    ...
    const sketch = (p: p5) => {
      ...
      p.setup = () => {
        ...
        // Prevent browser scrolling/zooming gestures on mobile
  ...
  return (
    <div className="relative w-full h-full min-h-[70vh]">
        <div
          ref={containerRef}
          className="w-full h-full absolute top-0 left-0 z-0 cursor-crosshair"
...
const drawConnectorLines = (p: p5, _cx: number, _cy: number, wx: number, wy: number, isMobile: boolean) => {
    const ctx = p.drawingContext as CanvasRenderingContext2D;
    const w = p.width;
    const h = p.height;
